...
export function SearchBar({ size = "lg", autoFocus = false }: { size?: "lg" | "md"; autoFocus?: boolean }) {
  ...
  const go = (r: SearchResult) => {
    setOpen(false);
    setQ("");
    navigate({ to: "/courses/$slug", params: { slug: r.courseSlug } });
  };
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {open && q && (
        <div className="absolute z-30 mt-2 left-0 right-0 rounded-2xl border border-[color:var(--color-brand-border)] bg-[color:var(--color-brand-surface)] shadow-2xl overflow-hidden">
          {results.length === 0 ? (
          ...
          <div className="border-t border-[color:var(--color-brand-border)] px-4 py-2 text-[11px] text-white/40 flex items-center justify-between">
            ...
            <Link to="/courses" className="text-[color:var(--color-brand-green)] hover:underline">Browse all →</Link>
          </div>
        </div>
      )}
    </div>
