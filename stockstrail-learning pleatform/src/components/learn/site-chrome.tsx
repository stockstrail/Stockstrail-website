...
export function LearnLogo({ className = "" }: { className?: string }) {
  return (
    <Link to="/" className={`flex items-center gap-2.5 ${className}`} aria-label="StocksTrail Learning home">
      ...
      <div className="flex items-baseline">
        <span className="text-white font-display text-[22px] sm:text-[26px] font-bold" style={{ fontFamily: "var(--font-display)" }}>Stocks</span>
        <span className="text-white font-display text-[22px] sm:text-[26px] font-normal" style={{ fontFamily: "var(--font-display)" }}>trail</span>
...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-[color:var(--color-brand-bg)]/85 border-b border-[color:var(--color-brand-border)]">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12 h-16 flex items-center justify-between gap-4">
        <LearnLogo />
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV.map((n) => (
            <Link
              ...
              activeProps={{ className: "text-[color:var(--color-brand-green)]" }}
        ...
        <div className="flex items-center gap-2">
          <a
            href="https://stockstrail.in"
            className="hidden sm:inline-flex items-center gap-2 rounded-full border border-[color:var(--color-brand-border)] px-4 py-2 text-xs font-medium text-white/80 hover:text-white hover:border-[color:var(--color-brand-green)]/60 transition-all"
          >
            Back to StocksTrail
      ...
      {open && (
        <div className="md:hidden border-t border-[color:var(--color-brand-border)] px-5 py-3 bg-[color:var(--color-brand-bg)]">
          <nav className="flex flex-col">
            ...
            <a href="https://stockstrail.in" className="py-2.5 text-sm text-white/70">Back to StocksTrail →</a>
          </nav>
        </div>
      )}
    </header>
...
export function SiteFooter() {
  return (
    <footer className="mt-24 border-t border-[color:var(--color-brand-border)]">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12 py-14 grid gap-10 md:grid-cols-4">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-white/70">
            <li><Link to="/courses" className="hover:text-[color:var(--color-brand-green)]">All Courses</Link></li>
            <li><Link to="/categories" className="hover:text-[color:var(--color-brand-green)]">Categories</Link></li>
            <li><Link to="/about" className="hover:text-[color:var(--color-brand-green)]">About</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="text-white text-sm font-semibold mb-3">StocksTrail</h4>
          <ul className="space-y-2 text-sm text-white/70">
            <li><a href="https://stockstrail.in" className="hover:text-[color:var(--color-brand-green)]">Main site</a></li>
            <li><a href="https://stockstrail.in/services" className="hover:text-[color:var(--color-brand-green)]">Services</a></li>
            <li><a href="https://stockstrail.in/contact" className="hover:text-[color:var(--color-brand-green)]">Contact</a></li>
          </ul>
        </div>
      </div>
